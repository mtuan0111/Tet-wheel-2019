function notificationMessage(text, title = "Thông báo", timeOutDestroy = 5) {
    ...
    var randomCircle = Math.round(Math.random() * 11) + 1;
    document.getElementsByClassName("campaignCircle")[0].style =
        "-webkit-mask-image: url(common/img/circle-" +
        pad(randomCircle, 2) +
        ".svg);\
...
notificationMessage.prototype.destroy = function() {
    ...
    setTimeout(function() {
        ...
        document.getElementsByClassName("campaignIcon")[0].style =
            "-webkit-mask-image: url(common/img/pig-icon-" +
            pad(randomImage, 2) +
            ".svg);\
      -webkit-mask-repeat: no-repeat;\
      -webkit-mask-size: 70%;\
