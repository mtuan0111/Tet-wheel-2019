    ...
    { deg: deg_rotate },
    {
      ...
      step: function(now) {
        ...
        var activePeace = Math.round(
          (360 - nowe) / rotate_each_point + horizontal_deg / rotate_each_point
        );

        // if(activePeace < 0){
        activePeace =
          activePeace < 0 ? number_of_point + activePeace : activePeace;
        $(_this.rotateWings).removeClass("active");
        $(_this.rotateWings[activePeace]).addClass("active");

        // }

        pointerRotate = -(nowe % number_of_point) + rotate_each_point / 10;
        if (pointerRotate < -(rotate_each_point / 2)) {
          pointerRotate = pointerRotate;
        } else pointerRotate = 0;

        pointerRotate = pointerRotate * 1.5 - 135;
        $(_this.pointer).css({
          "-webkit-transform":
            "translate(-50%, -100%) rotate(" + pointerRotate + "deg)",
          "-moz-transform":
            "translate(-50%, -100%) rotate(" + pointerRotate + "deg)",
          "-ms-transform":
            "translate(-50%, -100%) rotate(" + pointerRotate + "deg)",
          "-o-transform":
            "translate(-50%, -100%) rotate(" + pointerRotate + "deg)",
          transform: "translate(-50%, -100%) rotate(" + pointerRotate + "deg)"
        });
        ...
        var messageNotifi =
          "Xin chúc mừng, bạn đã quay vào ô " + point_value + " điểm.";
